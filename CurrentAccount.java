...
public class CurrentAccount extends Account {
	...
	public CurrentAccount(int accNo, String name, String location) {
		super(accNo, name);
		super.setType(2);
		this.balance=0;
		...
		try {
			...
			bw.write(name);
			bw.write("\n"+accNo+"\n");
			bw.write("Current Account\n");
			bw.write("\n\t\t\tBalance\t\t\tInterest\n");
			bw.write("Deposit\t\t"+df.format(0)+"\n");
	...
	public void deposit(double amt) {
		...
		details = sdf.format(old.getTime())+"\t"+df.format(balance)+"\t\t\t"+df.format(interest);
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(fileName, true));
